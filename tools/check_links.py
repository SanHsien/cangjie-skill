...
另外核對根目錄 `SKILL.md` 以反引號寫出的 `methodology/`、`extractors/`、
`templates/`、`agents/` 路徑，避免裝包缺檔。
...
SKILL_PATH_PATTERN = re.compile(
    r"`((?:methodology|extractors|templates|scripts|assets|agents)/[A-Za-z0-9._/-]+)`"
)
...
REQUIRED_SKILL_PREFIXES = ("methodology/", "extractors/", "templates/")
...
def check_skill_paths(skill_path: Path = SKILL) -> list[str]:
    ...
    missing_prefixes = [
        prefix for prefix in REQUIRED_SKILL_PREFIXES if not any(item.startswith(prefix) for item in seen)
    ]
    if missing_prefixes:
        problems.append(
            "SKILL.md 沒有點到必要裝包路徑：" + "、".join(missing_prefixes)
        )
    return problems
