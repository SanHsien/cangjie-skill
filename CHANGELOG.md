...
- **授權改 MIT。** 上游 `f751bf9` 把原作改成 MIT；本線 `LICENSE`、`NOTICE.md` 與 overlay 同步以 MIT 發佈。GitHub badge 改 MIT。
- **不跟該 commit 的其餘部分。** 繁中仍是主入口；不收 `README.ja.md`／`README.zh-CN.md`；不嵌入官網、QR、作者宣傳。

2026-08-27 審查當時選擇維持 AGPL，是因為授權不能跟宣傳包在同一筆自動 merge。次日維護者明確決定跟授權這一項。

## 2026-08-27（全庫審查；略過上游四筆；當時維持 AGPL）
