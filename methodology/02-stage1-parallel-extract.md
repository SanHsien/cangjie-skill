...
使用宿主提供的子 Agent 能力启动任务。Claude Code / Cursor 用 Agent / Task; Codex 可用当前会话的 `spawn_agent` / `wait_agent`,但必须先检查实际可用容量,不要假设固定槽位数。
...
每个子 Agent 只写自己的唯一产出文件。若处于嵌套调用、已有其他 Agent 或无法确认容量,应保守缩小批次、逐个运行或串行。
