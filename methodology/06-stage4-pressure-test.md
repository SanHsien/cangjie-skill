...
两项增强 (来自上游 PR #10,已核对 diff):

1. **执行质量测试 (`execution_check`)**: 检查 agent 调用 skill 后,是否真的按 E 段执行并忠于作者方法论。
2. **对抗性出题**: 优先让独立 agent 只看 name + description,生成容易误触发或漏触发的用户话术;没有可用 sub-agent 时由主流程补题并标记 fallback。

## 为什么必须做

A2 (trigger) 是拆书里最难的环节。一个 skill 做得再漂亮,trigger 不准就等于不存在。压力测试是**唯一**能在发布前发现 trigger 问题的方法。

只测 trigger 还不够。一个 skill 可能"该调用时确实调用了",却在执行时走偏。例如 E 段要求先识别反例,agent 却直接给建议。这类问题必须用 `execution_check` 测出来。
...
| `should_not_trigger` (诱饵) | 3–4 条 | 不该调用时是否忍住 |
...
**没有 `execution_check` 的 skill 一律打回**。因为 trigger 准确不代表执行正确。`execution_check` 是兼容扩展: 保留原有 `test_cases` 数组和 `type` 字段,只新增一种测试类型。

**跨 skill 混淆测试 (硬性要求)**: 诱饵中至少 2 条必须是"应该触发同书另一个 skill"的 prompt。同一本书拆出的 10+ 个 skill 之间互相抢调用,是部署后最常见的真实故障 — 只测"完全无关的场景"发现不了它。盲测时把整包所有 skill 的 name + description 列表给 sub-agent,让它做"该激活哪一个"的选择题,而不只是"要不要激活这一个"的判断题。

## 对抗性出题

在主流程写完基础测试集后,优先新增一个对抗性出题 agent。宿主没有可用 sub-agent 时,由主流程只根据 `name` + `description` 补题,并在 `test-results.md` 记录 `adversarial_mode: fallback_self_generated`:

1. 只给它当前 skill 的 `name` 和 `description`。
2. 不给它 R / I / A1 / E / B 正文,也不给预期答案。
3. 要求它生成 3 条用户 prompt: 1 条最容易误触发、1 条最容易漏触发、1 条容易和同书相邻 skill 混淆。
...
本阶段结束时暂存测试用例数、四类测试分布、对抗题数、跨 skill 混淆数、首轮/最终通过率、诱饵失败数、执行质量失败数、回炉次数、是否独立盲测与对抗出题模式;阶段 5 选择生成 `SCORECARD.md` 时再汇总。

## 下一步

所有 skill 全部通过后,进入阶段 5 (交付),见 `07-stage5-deliver.md`: 生成面向读者的 DIGEST.md,按需生成 SCORECARD.md,并把 skill 安装到用户的 skills 目录 — 之后才向用户提 darwin-skill 自动进化。
