...
本文是 cangjie-skill 所用 SOP 的设计说明,解释"为什么这么做"。执行时的具体步骤请读 `SKILL.md` 和 `methodology/00.5-*` 至 `07-*`。
...
| 随风《蒸馏读书法》 + 上游 PR #4/#22 | 阶段 0.5 适配预筛选: 五维评分、复用检查、成本预期、三个坑 |
...
```
          ┌───────────────────┐
          │ 阶段 0.5: 预筛选   │  BOOK_FIT.md (评分 + 复用 + 成本)
          └─────────┬─────────┘
                    │ full / partial / alternate / not_suitable
                    ▼
          ┌───────────────────┐
          │ 阶段 0: 整书理解   │  Adler 四步
          └─────────┬─────────┘
                    ...
                    ▼
          ┌───────────────────┐
          │ 阶段 1: 并行提取   │  按宿主可用并发容量分批
          └─────────┬─────────┘
                    │ candidates/
                    ▼
          ┌───────────────────┐
          │ 阶段 1.5: 三重验证 │  成本感知评审 + V1 自适应
          └─────────┬─────────┘
                    │ 通过单元 + rejected/
                    ▼
          ┌───────────────────┐
          │ 阶段 1.6: 晋级门   │  promoted / router + 入口软预算 8
          └─────────┬─────────┘
                    │ 只有 promoted 往下建 skill
                    ▼
          ┌───────────────────┐
          ...
          └─────────┬─────────┘
                    ...
                    ▼
          ┌───────────────────┐
          │ 阶段 4: 压力测试   │  trigger + execution_check + 对抗题
          └─────────┬─────────┘
                    │
                    ▼
          ┌───────────────────┐
          │ 阶段 5: 交付       │  DIGEST.md + 可选 SCORECARD.md + 安装
          └───────────────────┘
                    │
                    ▼
...
3. **适配性**: 只有 full / partial fit 才默认进入完整拆书;其他类型必须先改产物形态或让用户确认
4. **可验证**: 每个 skill 必须通过三重验证 + 压力测试 (含 execution_check)
5. **可进化**: 每个 skill 必须附带 darwin 兼容的 test-prompts.json
6. **用户参与**: 阶段 0.5 默认执行,可由用户明示跳过;阶段 0 之后确认骨架,阶段 1.5 之后确认入选名单
7. **可交付**: 流程终点是"用户能调用"— skill 必须被安装到 skills 目录,读者需求由 DIGEST.md 承接;完整包、比较实验或审计需求再用 SCORECARD.md 记录流程健康度
