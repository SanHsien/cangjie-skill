...
至少 1 个步骤必须带 🔴 CHECKPOINT 视觉标记,且标记后有明确动作 (停下来问用户 / 展示给用户确认)。仅靠"必须"措辞不算 CHECKPOINT。
...
B 段必须包含"Agent 执行反模式"小节,至少 3 条"本 skill 激活后不要做的事",每条要有明确动作。

A2 段末尾必须有 ASCII「快速触发决策树」,覆盖不激活 / 激活,并至少 2 个指向兄弟 skill 的分支。语言信号本身就是触发词;不要另开 frontmatter 字段。
...
跨宿主兼容要求: frontmatter 只保留 `name` 与 `description`。来源、章节、标签、触发词和相关 skill 放入正文的"审计信息"与"相关 skills"部分,避免不同宿主解析扩展字段时产生兼容性差异。
